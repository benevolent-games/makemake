...
export function makeAerialCamera({ theater, cursor, inputs, mapSize, radius, sensitivity, sampleHeight, }) {
    let targetRadius = radius.initial;
    let targetHeight = 0;
    const minimumHeightOffGround = 10;
    const camera = new ArcRotateCamera("cam", 0.25 * circle, 0.15 * circle, targetRadius, v3.toBabylon([0, targetHeight, 0]), theater.scene);
    ...
    function updateTargetHeight() {
        targetHeight = sampleHeight(camera.globalPosition.x, camera.globalPosition.z);
    }
    ...
    function smoothUpdateForCameraHeight() {
        ...
        const floor = sampleHeight(camera.globalPosition.x, camera.globalPosition.z) + minimumHeightOffGround;
    ...
    cursor.listeners.mousemove.add(({ movementX, movementY }) => {
        if (isPressingPanButton()) {
            camera.target.x = cap(camera.target.x + (movementX * (camera.radius / 200)), boundaryA, boundaryB);
            camera.target.z = cap(camera.target.z - (movementY * (camera.radius / 200)), boundaryA, boundaryB);
            updateTargetHeight();
        }
    });
    theater.renderLoop.add(() => {
        if (!isPressingPanButton()) {
            ...
            if (px > 0.95)
                ...
                movementX -= 1;
            if (movementY !== 0)
                camera.target.z = cap(camera.target.z - (20 * movementY * (camera.radius / 200)), boundaryA, boundaryB);
            if (movementX !== 0)
                camera.target.x = cap(camera.target.x - (20 * movementX * (camera.radius / 200)), boundaryA, boundaryB);
            updateTargetHeight();
        }
    });
    return camera;
}
//# sourceMappingURL=aerial-camera.js.map